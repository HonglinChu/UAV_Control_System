/**
  ******************************************************************************
  * @file    EEPROM_Emulation/src/eeprom.c       模拟EEPROM
...
/* 存储虚拟变量的地址，不能将地址定义为0xffff*/
extern uint16_t VirtAddVarTab[NumbOfVar];

/* Private function prototypes -----------------------------------------------*/
/* Private functions ---------------------------------------------------------*/
static FLASH_Status EE_Format(void);
static uint16_t EE_FindValidPage(uint8_t Operation);       /* 寻找有效FLASH页 */
static uint16_t EE_VerifyPageFullWriteVariable(uint16_t VirtAddress, uint16_t Data);  /* 判断页是否满和写入变量值 */
static uint16_t EE_PageTransfer(uint16_t VirtAddress, uint16_t Data);   /* 页面间数据的传递 */
...
static uint16_t EE_FindValidPage(uint8_t Operation)
{
  ...
  switch (Operation)
  {
    case WRITE_IN_VALID_PAGE:   /* ---- Write operation ---- */
      if (PageStatus1 == VALID_PAGE)  			/* VALID_PAGE 为存储有效数据的页面 */
      {
