...
uint32_t Micros(void)
{
    ...
    } while (ms != sysTickUptime);//误差us级别。
	value=ms * 1000 + (9000 - cycle_cnt)/9;
    return value;
}
void delay_init()
{
  SysTick_CLKSourceConfig(SysTick_CLKSource_HCLK_Div8);//选择外部时钟HCLK/8=9MHz，//默认是72MHz　　
  // SysTick_Config(SystemCoreClock / 8000);//SysTick开启系统tick定时器并初始化其中断，1ms一个中断   
  SysTick->CTRL|=SysTick_CTRL_TICKINT_Msk;   	//开启SYSTICK中断
  SysTick->LOAD=SystemCoreClock/8000-1; 		//每1ms中断一次	,,赋值9000，，，频率9Mhz
...
//	fac_us=SystemCoreClock/8000000;				//为系统时钟的1/8  每1us要计数9次
//#if SYSTEM_SUPPORT_OS  							//如果需要支持OS.
