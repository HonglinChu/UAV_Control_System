...
void BMP085_Routing(void)
{
  switch(BMP085_ST)
  {
      ...
      case CTemperatureing: 
				...
				BMP085_ST=SCPressure;
  				break;
      case SCPressure:  
  				BMP085_writemem(CONTROL, READ_PRESSURE+(_oss << 6));
				BMP085_ST=SCPressureing;
  				break;
      case SCPressureing:  
				  BMP085_getAltitude(&last_Alt,0);
	              if(BAR_Calibration_Flag)
				  {
					 LED0=0;
				     BMP085_Calibration();
				  }
				  BMP085_newALT(last_Alt);//对高度进行滤波
				  BMP085_GetAlt(&ALT_BAR);//ALT_BAR上传数据的更新
				  BMP085_ST=SCTemperature;
				  break;
  default :BMP085_ST=SCTemperature; break;
...
void BMP085_writemem(uint8_t _addr, uint8_t _val) {
  IIC_Write_Byte(BMP085_ADDR,_addr,_val);
}

void BMP085_readmem(uint8_t _addr, uint8_t _nbytes, uint8_t __buff[]) {
  IIC_Read_Len(BMP085_ADDR,_addr,_nbytes,__buff);
}
...
void BMP085_calcTrueTemperature(u8 rw){
  ...
  if(rw)
  { 
    BMP085_writemem(CONTROL, READ_TEMPERATURE);//发送读取温度的命令
...
{
  int32_t TruePressure;
  BMP085_calcTruePressure(&TruePressure,rw);//计算真是的压力值 
  BMP085_newPressure(TruePressure);         //对压力值进行过滤
...
void BMP085_Calibration(void)
{
   ...
   if(time==50)
   {
      _cm_Offset=(temp+25)/50;
	  time=0;
	  temp=0;
	  BAR_Calibration_Flag=0;
	  LED0=1;
	  EE_Save_Bar_Offset();
	  return;
   }
